...
env = Environment("Simulation2d/svg/dac28e8e324100b7fd7c14c841f79148_out", 12)
env.use_observation_rotation_size(True)
env.set_observation_rotation_size(128)

env.use_observation_adjacent_use(False)
...
def build_agent(model, actions):
	...
	dqn = DQNAgent(model=model, memory=memory, policy=policy, processor=processor,
		nb_actions=actions, nb_steps_warmup=100, target_model_update=5_000,
		enable_double_dqn=True,
		enable_dueling_network=True, dueling_type='avg',
		batch_size=16, gamma=.99)
	return dqn

model = build_model(states, actions)
dqn = build_agent(model, actions)
callbacks = build_callbacks('dac28e8e324100b7fd7c14c841f79148_out_12')
