...
env = Environment("Simulation2d/svg/01e53c5618fd8d4d78c916b9dcc4ff92_out", 6)
...
while not done:
	...
	state = list(map(lambda x: x * 10, state))

	env.render()

	angles = []
	distances = []
	for i in range(len(state) - env.get_observation_rotation_size()):

		_deg = float((0.25*i) - 135)
		if _deg < 0:
			_deg = 360 + _deg
		#print(math.radians(_deg))
		angles.append(float(math.radians(_deg)))
